...
public class UserControllerTest {
    ...
    @Test
    void handleGetRequestWithMatchingToken1() {
        // Arrange
        UserController userController = spy(UserController.class); // Make sure UserController is accessible
        Response response = mock(Response.class);

        Request getRequest = createMockRequest(Method.GET, "/users/testUser", "testUser");

        doReturn(response).when(userController).handleRequest(getRequest); // Make sure handleRequest method exists in UserController
